...
이 구조에서 가장 먼저 체감하는 효익이다. 각 모듈의 책임이 컴파일 타임에 강제되기 때문에, "어디를 고쳐야 하지?"를 고민하는 시간이 거의 사라진다.

- `boot/ma-boot-web`: 클라이언트와의 입출력 (Controller, Request/Response, Security 필터)
- `domain/ma-domain-core`: 비즈니스 로직 (도메인 객체, 도메인 서비스, 포트)
- `infrastructure/storage/ma-db-core`: 데이터 접근 로직 (테이블 정의, 쿼리, 어댑터)

요구사항이 들어왔을 때의 사고 흐름이 자연스럽게 정해진다.

- "응답 필드 추가" → web 모듈만
- "비즈니스 규칙 변경" → domain 모듈만
- "쿼리 최적화/인덱스 추가" → db 모듈만
- 이 셋이 동시에 바뀌면 → "기능이 정말 그만큼 큰 거구나"라는 신호

빈혈 모델 + 단일 모듈 프로젝트에서 흔한 "Service에 다 들어 있고, Controller에서 비즈니스 로직 한 줄, Repository에 또 한 줄" 같은 산발적 수정이 사라진다. **변경 지점이 모듈 경계와 일치**한다.

### 2. 동일 기능을 여러 번 만들 필요가 없다 (다중 클라이언트 대응)

이 부분은 **JPA Entity 기반 설계와 가장 크게 갈리는 지점**이다.

같은 도메인 데이터를 여러 채널로 노출해야 하는 경우가 흔하다.

- 웹 화면용 응답 (요약 카드: id, title, author)
- 모바일 화면용 응답 (상세 + 좋아요 수 + 미리보기 댓글)
- 데이터 다운로드용 응답 (모든 컬럼 + 통계)

JPA에서는 같은 Entity를 가지고 채널마다 필요한 필드를 노출하려고 하면 Lazy Loading 함정에 빠지기 쉽다.

- 화면 A에서는 연관 엔티티가 필요해서 fetch join을 걸어야 하고
- 화면 B에서는 그 연관이 필요 없는데, Entity 그래프가 통째로 딸려 오거나 N+1이 터지고
- 화면 C는 통계 컬럼이 필요해서 결국 별도 쿼리/DTO를 또 만들고
- "OSIV로 어떻게든 돌리자" → 트랜잭션/커넥션 점유 문제로 다시 돌아옴

순수 도메인 모델 + Exposed 같은 명시적 SQL 도구를 쓰면 접근 방식이 달라진다.

- 도메인은 도메인대로 비즈니스 규칙만 책임진다
- DB 모듈은 **각 채널이 요구하는 형태에 맞는 별도의 조회 메서드/Projection**을 명시적으로 만든다
- 화면별 응답 DTO는 web 모듈에서 도메인(또는 Projection) → DTO 매핑으로 만든다

결과적으로 "한 화면에 맞춰 만든 쿼리가 다른 화면을 망가뜨리는" 일이 사라진다. 각 채널은 자기에게 필요한 만큼만 가져가고, 도메인은 어느 채널에서 호출되든 동일한 비즈니스 규칙을 보장한다. **클라이언트가 늘어날수록 이 구조의 가치가 커진다.**
